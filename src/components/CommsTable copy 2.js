...
const CommsTable = ({ simbriefData }) => {
  ...
  useEffect(() => {
    ...
    const metarRaw = simbriefData.destination?.metar;
    const metar = typeof metarRaw === 'string' ? metarRaw : '';
    const match = metar.match(/Q\d+/);
    const qnh = match ? match[0].replace('Q', '') : 'Sem QNH';
    ...
    <div className="mt-8 p-4">
      ...
      <Card className="p-4 mb-4 w-full">
        ...
        <div className="grid grid-cols-2 gap-2">
          {['trafego', 'Solo', 'Torre', 'Aproximação', 'Centro'].map((service) => (
